...
/** Adds the Extension Addons to Oritech's own "machines" creative tab. */
...
public class ModEvents {
    ...
    private static final ResourceKey<CreativeModeTab> ORITECH_MACHINES = ResourceKey.create(
            ...
            Identifier.fromNamespaceAndPath("oritech", "machine_group"));

    @SubscribeEvent
    static void addToOritechTab(BuildCreativeModeTabContentsEvent event) {
        if (event.getTabKey().equals(ORITECH_MACHINES)) {
            event.accept(OritechAddonsOne.EXTENSION_ADDON_1_ITEM.get());
            event.accept(OritechAddonsOne.EXTENSION_ADDON_2_ITEM.get());
            event.accept(OritechAddonsOne.EXTENSION_ADDON_3_ITEM.get());
        }
    }
}
